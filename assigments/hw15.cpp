...
void move (int from, int to, int n){
    ...
    int temp= tower[from].back();
    //for odd number of rings
    if (n%2!=0){
        while (tower[1].size() < n+1){
            cout << "Step " << ++move << ": Move ring " << temp << " from tower " << from+1 << " to tower " << to+1 << endl;
